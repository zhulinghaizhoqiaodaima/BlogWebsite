...
function Detail(props: any) {
  ...
  const starAdd = ()=>{
    ...
    UpdateNews(id, {
     ...
     setnewInfo({...newInfo,star:++newInfo.star})
     setisStar(true)
      ...
      }}>
        {
          newInfo && (
            <PageHeader
              ...
              subTitle={
                <div>
                  <span>{newInfo.category.title}</span>
                  <span>{`  `}</span>
                  <span onClick={()=>starAdd()}>
                  <HeartIcon style={{ color: isStar ?  'hotpink':'#F1F3F4' }}/>
                  </span>
                  {/* <HeartTwoTone twoToneColor="#eb2f96" onClick={()=>starAdd()} /> */}
                </div>
              }
            >
